  ...
  return (
    ...
    <div className="fixed inset-x-4 bottom-4 z-50 flex flex-col items-end sm:inset-x-auto sm:bottom-6 sm:right-6">
      ...
      {isOpen && (
        ...
        <div className="mb-4 w-full sm:w-100 h-[calc(100dvh-120px)] max-h-150 sm:h-137.5 bg-base/95 rounded-2xl shadow-[0_0_30px_rgba(var(--rgb-glow),0.15)] border border-[rgba(var(--rgb-film),0.08)] overflow-hidden flex flex-col backdrop-blur-xl transition-all duration-300 transform origin-bottom-right animate-in zoom-in-95">
          
          <div className="bg-linear-to-r from-surface to-[rgba(var(--rgb-glow),0.05)] border-b border-[rgba(var(--rgb-film),0.08)] p-4 text-text-primary font-semibold flex justify-between items-center">
            ...
            {messages.map((msg, i) => (
              <div key={i} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                <div className={`p-3.5 max-w-[85%] text-sm leading-relaxed shadow-sm ${
                  msg.role === 'user' 
                    ? 'bg-linear-to-br from-accent-purple to-[#5a38a3] text-white rounded-2xl rounded-tr-sm whitespace-pre-wrap' 
                    : 'bg-[rgba(var(--rgb-film),0.04)] text-text-primary rounded-2xl rounded-tl-sm border border-[rgba(var(--rgb-film),0.05)] whitespace-pre-wrap'
                }`}>
                  {/* Emphasis inherits the bubble's text colour — a fixed white
                      here disappears against the light theme's bubbles. */}
                  <span dangerouslySetInnerHTML={{ __html: msg.text.replace(/\*\*(.*?)\*\*/g, '<strong class="font-semibold">$1</strong>').replace(/\n/g, '<br/>') }} />
                </div>
                  ...
                  <div className="flex flex-col gap-2 mt-4 max-w-full sm:max-w-[95%]">
                    {QUICK_PROMPTS.map((prompt, idx) => (
                      <button
                        ...
                        className="text-xs border border-[rgba(var(--rgb-glow),0.3)] bg-[rgba(var(--rgb-glow),0.05)] text-accent-lavender px-4 py-2.5 rounded-xl hover:bg-[rgba(var(--rgb-glow),0.15)] hover:border-[rgba(var(--rgb-glow),0.6)] hover:translate-x-1 transition-all duration-200 text-left leading-relaxed shadow-sm"
                      >
                        {prompt}
                      </button>
                    ))}
            ...
            {isLoading && (
              <div className="flex items-center gap-2 text-xs text-accent-purple font-medium ml-2 animate-pulse">
                <span className="flex gap-1">
                  ...
                  <span className="h-1.5 w-1.5 bg-accent-purple rounded-full animate-bounce" style={{ animationDelay: '300ms' }}></span>
                </span>
          ...
          <div className="p-3 bg-[var(--app-panel-strong)] border-t border-[rgba(var(--rgb-film),0.08)] backdrop-blur-md">
            {isLimitReached ? (
              <div className="text-center py-3 text-sm text-text-secondary bg-[rgba(var(--rgb-film),0.02)] rounded-xl border border-[rgba(var(--rgb-film),0.05)]">
                <p className="mb-1.5 font-medium text-text-primary">Sesi percakapan limit! 🚀</p>
              ...
              <div className="flex gap-2 items-end">
                <textarea 
                  ...
                  className={`flex-1 bg-surface border rounded-xl px-4 py-2.5 text-sm text-text-primary focus:outline-none transition-colors resize-none overflow-y-auto leading-relaxed [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] scrollbar-none ${
                    cooldown > 0 
                      ? 'border-yellow-500/30 placeholder-yellow-500/70 cursor-not-allowed' 
                      : 'border-[rgba(var(--rgb-film),0.08)] placeholder-text-muted focus:border-accent-purple focus:ring-1 focus:ring-accent-purple/50'
                  }`}
                  placeholder={
                    cooldown > 0 
                ...
                <button 
                  ...
                  className={`px-4 sm:px-5 rounded-xl h-11 text-sm font-semibold transition-all duration-300 flex items-center justify-center ${
                    isLoading || cooldown > 0 || !input.trim()
                      ? 'bg-[rgba(var(--rgb-film),0.05)] text-text-muted cursor-not-allowed'
                      : 'bg-linear-to-r from-accent-purple to-[#603ba8] text-white hover:shadow-[0_0_15px_rgba(var(--rgb-glow),0.4)] hover:scale-[1.02]'
                  }`}
                >
                  {cooldown > 0 ? (
      ...
      {!isOpen && isHome && (
        <div 
          className={`mb-4 mr-2 max-w-55 p-3 bg-[var(--app-panel-strong)] border border-[rgba(var(--rgb-glow),0.4)] rounded-2xl rounded-br-sm shadow-[0_0_20px_rgba(var(--rgb-glow),0.2)] backdrop-blur-md transition-all duration-500 ease-in-out transform origin-bottom-right flex items-center gap-2 cursor-pointer ${
      ...
      <button 
        ...
        className="w-14 h-14 bg-linear-to-tr from-accent-purple to-[#a27df0] rounded-md text-white flex items-center justify-center hover:scale-110 transition-all duration-300 shadow-[0_0_20px_rgba(var(--rgb-glow),0.4)] relative"
      >
        {isOpen ? (
